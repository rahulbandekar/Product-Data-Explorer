...
export default function CategoryPage() {
  ...
  const scrapeProducts = useScrapeProducts(categoryId);

  useEffect(() => {
    if (
      !productsLoading &&
      !scrapeProducts.isPending &&
      products.length === 0 &&
      !hasAutoScraped.current &&
      category // Make sure category exists
    ) {
      hasAutoScraped.current = true;
      handleScrapeProducts();
    }
  }, [productsLoading, products.length, scrapeProducts.isPending, category]);

  const handleScrapeProducts = async () => {
    try {
      await scrapeProducts.mutateAsync(true);
      setTimeout(() => refetchProducts(), 4000);
    } catch (err) {
              ...
              <p className="mt-2 text-gray-600">
                ...
                {category.lastScrapedAt && (
                  <span className="ml-4 text-sm text-gray-500">
                    Last updated:{" "}
                    {new Date(category.lastScrapedAt).toLocaleDateString()}
                  </span>
                )}
              </p>
            </div>
          ...
          {!productsLoading && products.length === 0 && (
            <div className="text-center py-12 bg-white rounded-lg border-2 border-dashed border-gray-300">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Try refreshing the category data or check back later.
              </p>
              <div className="mt-6">
                <button
                  onClick={handleScrapeProducts}
